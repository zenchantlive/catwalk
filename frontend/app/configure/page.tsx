...
// Removed unused FormSchema import
...
function ConfigureContent() {
    ...
    const mutation = useMutation({
        ...
        onError: (err: Error & { response?: { data?: { detail?: DeploymentError } }; detail?: DeploymentError }) => {
            console.error("Deployment error:", err);
            // Extract structured error from response
            // The error detail could be in err.response.data.detail or err.detail
            const errorDetail: DeploymentError = (err?.response?.data?.detail as DeploymentError) || (err?.detail as DeploymentError) || {
