...
import { SettingsService, type SettingsResponse } from "@/lib/settings-service"
...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                ...
                setSettings(data)
                setFlyToken(data.fly_api_token || "")
    ...
    try {
        ...
        // Only send keys if the user has entered something (non-empty).
        // If empty, we assume they want to keep the existing key (if any).
        // To delete keys, they should use the "Clear All" button.
        const payload: any = {}
        if (flyToken) payload.fly_api_token = flyToken
        if (openRouterKey) payload.openrouter_api_key = openRouterKey

        const updated = await SettingsService.updateSettings(payload)
        setSettings(updated)
        setFlyToken(updated.fly_api_token || "") // Typically API might mask it, but here it returns decrypted for editing. 
        // Actually backend returns decrypted if we send it or if we request it.
...
const handleDeleteAll = async () => {
    ...
    try {
        ...
        // We can just reset state here since we know it's empty
        setLoading(false)
            ...
            <div className="grid gap-6">
                <Card className="border-0 shadow-sm ring-1 ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Provide your API keys to enable deployment and AI features.
                            Keys are stored securely with Fernet encryption.
                        </CardDescription>
                    </CardHeader>
                    ...
                    <CardFooter className="flex flex-col gap-4 border-t bg-gray-50/50 p-6 dark:bg-gray-900/50 sm:flex-row sm:justify-between sm:items-center">
                        <Button
                            variant="ghost"
